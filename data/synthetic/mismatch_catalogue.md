...
| 7 | **fx_rounding** | gateway charges in USD/EUR; invoice booked in INR at the "true" rate (`FX_TO_INR`); the bank settled at a slipped rate (`BANK_FX_SLIP`) | conversion-timing differences | `escalated`. The LLM correctly identifies the counterpart (same ref/date/customer) with high confidence, but the residual rupee variance is a forex gain/loss call for a human — so it escalates with the LLM's finding attached, not auto-resolved |
